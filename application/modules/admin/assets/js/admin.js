$('body').ready(function() {
    ...
    $('body').on('click', '#_submit_admin_form', function(){
	    ...
	    if(required == 0){
	    	var id = $('#admin_id').val();
			var titre = (id == '')?'Ajout admin':'Modification admin';
			var datas = {"titre":titre,"id":$('#admin_id').val(),"civil":$('#civil').val(),"nom":$('#nom').val(),"prenom":$('#prenom').val(),"email":$('#email').val(),"login":$('#login').val(),"password":$('#password').val(),"old_pass":$('#old_pass').val()};
