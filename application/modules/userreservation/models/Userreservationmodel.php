...
class UserreservationModel extends MY_Model
{
	...
	function getReservationListSearch($search='',$searchstart='',$searchend='')
	{
		...
		$this->db->join('client c','c.id = r.id_client');
		$criteres = "c.id != 0";
		if ($search != '') $criteres .= " AND (r.id LIKE '%".$search."%' OR r.scenario LIKE '%".$search."%' OR r.jour LIKE '%".$search."%' OR c.nom LIKE '%".$search."%' OR c.prenom LIKE '%".$search."%' OR c.email LIKE '%".$search."%')";
