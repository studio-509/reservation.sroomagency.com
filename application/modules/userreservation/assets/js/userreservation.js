$(document).ready(function() {
	...
	*/
	$('body').on('click', '._comment_resa', function() {
		...
		var datas = {'id':id};
		_inPop.open('/userreservation/admin/Userreservationadmin/loadComment', datas, 'success');
	...
	*/
	$('body').on('click', '._societe_resa', function() {
		...
		$('html, body').animate({ scrollTop: 0 }, 'fast');
		var datas = {'id':id};
		_inPop.open('/userreservation/admin/Userreservationadmin/loadCompanyinfos', datas, 'success');
	...
	*/
	$('body').on('click', '._reservation_update', function(){
		...
		temp = _Ajax.callback('/userreservation/admin/Userreservationadmin/getResaInfos', datas, 'fn_reservation');
		_Ajax.reload('/userreservation/admin/Userreservationadmin/infos_up','info_resa' ,datas);
			_Ajax.reload('/userreservation/admin/Userreservationadmin/calendar', 'calendar', datas);
	...
	*/
	$('body').on('click', '._cal_drop', function(){
		...
		// var link = (origine == 'admin')?'admin/Userreservationadmin':'Reservation';
		if (origine == 'admin'){
			var link = 'admin/Userreservationadmin';
		...
		_Ajax.reload('/userreservation/' + link + '/calendar', 'calendar', datas);
	});
	/**
	* jump jour
	*/
	$('body').on('click', '._cal_drop_mobile', function(){
		...
		_Ajax.reload('/userreservation/' + link + '/mobileCalendar', 'calendar', datas);
	});
	/**
	* jump salles front
	*/
	$(document).on('change','#salle',function(){
		...
		if (origine == 'admin'){
			_Ajax.reload('/userreservation/Userreservation/getNbmax', 'sel', datas);
			_Ajax.reload('/userreservation/' + link + '/' + mobile, 'calendar', datas);
		}
		if(origine == 'front'){
			_Ajax.reload('/userreservation/Userreservation/index','_calendrier',datas);
			_Ajax.change_uri('/userreservation/Userreservation/changeUri', datas);
		}
	});
	 ...
	 */
 	$('body').on('touchend click', '._mobile_front_resa', cancelDuplicates(function(){
		...
		jQuery.ajax({
			type: 'POST',
			url: '/userreservation/Userreservation/changeMobileUri',
			data: {
				'salle': id,
				'start':maintiendate,
				'nbjoueurs':nbjoueurs,
	...
	*/
	$('#salle-admin').change(function(){
		...
		jQuery.ajax({
  			type: 'POST',
  			url: '/userreservation/admin/Userreservationadmin/getNbmax',
  			data: {
	  			'message':message
  			},
  			success: function(data, textStatus, jqXHR) {
		...
		_Ajax.reload('/userreservation/admin/Userreservationadmin/calendar', 'calendar', {'start':$('.toto').attr('data-day'),'salle':$('#salle-admin option:selected').val(),'addIndispo':addIndispo,'addResa':addResa});
	});
	/**
	...
	**/
	$('body').on('change','#joueurs',function(){
		...
		if ($('#jour_resa').val() != '' && $('#horaire_resa').val() != '' && joueurs !='0' && $('input[name=tarifstand]:checked').val()=='1') {
			...
			jQuery.ajax({
				type: 'POST',
				url: '/userreservation/admin/Userreservationadmin/calculPrix',
				data: {
					'message':message
				},
				success: function(data, textStatus, jqXHR) {
	...
	*/
	$('body').on('click', '#_submit_resa_admin_form', function(){
		...
		var comment_societe = ($('#comment').val().length)?$('#comment').val():'';
		...
		if(required == 0 && format_email == 0 && error == 0 && envoimail != '' && joueurs != 0){
			...
			if (id == '') {
				...
				_Ajax.send('/userreservation/admin/Userreservationadmin/' + link, datas);
			}
		}
	...
	$('body').on('click', '._reservation_paid', function(){
		var id = $(this).attr('data-id');
		var datas = {"id":id};
		_Ajax.send('/userreservation/admin/Userreservationadmin/paid', datas);
	...
	*/
	$('body').on('click', '._front_resa', function(){
		...
		if (joueurs == 0) {		
			...
			_inPop.open('/userreservation/Userreservation/wrong_player_number', datas, 'alerte');
			//$('#_tr'+id).slideUp(500);
		}
		else {
			if ($('#id_resa_maintien_modif').val() != '') {
				_inPop.open('/userreservation/Userreservation/modif', {"titre":"Modification données de réservation","jour":jour,"horaire":horaire,"nbJoueurs":joueurs,"salle":salle,"id":resaid,"typemodif":"modif_cal"});
			}
			else {
			_inPop.open('/userreservation/Userreservation/form', datas);
			}
...
/* 	$('body').on('click', '#_submit_resa_front_form', function(){
		...
		if(required == 0 && format_email == 0){
			...
			_Ajax.send('/userreservation/add', datas);
		}
	}); */
	$('body').on('click', '#_submit_resa_front_form', function(){
		///
	...
	*/
	$('body').on('click', '._order_process', function(){
		...
		if($('#order_cgv').prop('checked') === false){
			...
			_Ajax.order('/userreservation/order', {"id":$(this).attr('data-id')}, 'fn_reservation');
		}
	});
	...
	*/
	$('body').on('click', '._modif_resa', function(){
		...
		_inPop.open('/userreservation/Userreservation/modif', {"id":$(this).attr('data-id'),"typemodif":$(this).attr('id')});
		}
	});
	/**
	...
	**/
	$('body').on('change','._pag_setup', function(){
		...
		_Ajax.reload('/userreservation/admin/Userreservationadmin/listing','bloc_resa_liste',datas);
		// $('#bloc_resa_cal').hide();
	});
	...
	*/
	$('body').on('click','._add_indispo',function(){
		...
		//	$('<div id="annulindispo"><a class="button btn_L btn_annul mt32" href="/admin/userreservations">Annuler</a>  </div>').appendTo('#cancel_area');
		//}
		$('.calday').css({
			'cursor':'pointer',
	...
	*/
	$('body').on('click','._annul_resaindis',function(){
		...
		$('#info_resa').html('');
		_Ajax.reload('/userreservation/admin/Userreservationadmin/calendar','calendar',{'start':$('.toto').attr('data-day'),'salle':$('#salle-admin option:selected').val(),'addIndispo':0,'addResa':0});
	});
	...
	*/
	$('body').on('keypress', '#searchresa', function() {
		...
		setTimeout(function() {
			...
			_Ajax.reload('/userreservation/admin/Userreservationadmin/listingsearch','tab_resa_search',datas);
		}, delayInMilliseconds);
	...
	$('body').on('change','._check_fiche',function() {
		...
		if ($(this).is(':checked')) {
			if (!$('#sheet_create').attr('href')) {
				$('#sheet_create').attr('href','https://'+window.location.host+'/userreservation/admin/Userreservationadmin/printfiche/');
...
var fn_reservation = {
	add : function(){
		...
		_inPop.open('/userreservation/admin/Userreservationadmin/add',datas);
	},
	setIndispo : function(indispo){
		...
		_inPop.open('/userreservation/admin/Userreservationadmin/setIndispo',datas,'alerte');
	},
	del : function(id){
		var datas = {"titre":"Suppression reservation","txt":"La reservation à bien été supprimée","id":id};
		_inPop.open('/userreservation/admin/Userreservationadmin/delete', datas, 'alerte');
		$('#_tr'+id).slideUp(500);
	},
	delindispo : function(id){
		var datas = {"titre":"Suppression indisponibilité","txt":"L'indisponibilité à bien été supprimée","id":id};
		_inPop.open('/userreservation/admin/Userreservationadmin/deleteIndispo', datas, 'alerte');
		$('#_tr'+id).slideUp(500);
	},
	...
	cancelflo : function(id){
		...
		_Ajax.reload('/userreservation/Userreservation/index','_calendrier',{"salle":''});
		_Ajax.change_uri('/userreservation/Userreservation/changeUri', {"salle":''});
	},
	reload : function(id){
		var datas = {"id":id};
		_inPop.open('/userreservation/reload', datas);
	},
