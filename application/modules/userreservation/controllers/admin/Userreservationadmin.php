...
class Userreservationadmin extends MY_Controller
{
	function __construct()
	{
		parent::__construct();
		$this->load->model('UserreservationModel');
	...
	public function listing($offset = 0)
	{
		...
		$this->data['reservations'] = $this->UserreservationModel->getReservationList();
		$this->data['salles'] = $this->UserreservationModel->getList('salle');
...
/* 		$pag = PAGINATION;
		...
		$this->data['nbRes'] = $this->UserreservationModel->getNbData('reservation', 'id_client != 0');
		...
		if ($resindis == 'resa') {
			$this->data['reservations'] = $this->UserreservationModel->getReservationList($limit);
		}
		else {
			$this->data['reservations'] = $this->UserreservationModel->getIndispoList($limit);
		}
		$this->data['salles'] = $this->UserreservationModel->getList('salle');
		$this->data['pag'] = $pag;
		$this->data['resindis'] = $resindis;
	...
	public function listingsearch()
	{
		...
		$this->data['reservations'] = $this->UserreservationModel->getReservationListSearch($data['search'],$data['searchstart'],$data['searchend']);
		$this->data['salles'] = $this->UserreservationModel->getList('salle');
	...
	public function delete()
	{
		$post = $this->input->post('message');
		$data = json_decode($post,TRUE);
		$infos = $this->UserreservationModel->getData('reservation', array('id' => $data['id']));
		$vérification = $this->UserreservationModel->getNbData('reservation', array('id_client' => $infos->id_client));
		$this->UserreservationModel->deleteData('reservation', array('id' => $data['id']));
		if($vérification == 1)
		{
			$this->UserreservationModel->deleteData('client', array('id' => $infos->id_client));
		}
		if($infos->voucher != '0')
		{
			$voucher = $this->UserreservationModel->getData('voucher', ['code' => $infos->voucher]);
			$this->UserreservationModel->updateData('voucher', ['valide' => '1'], ['id' => $voucher->id]);
		}
		$this->UserreservationModel->deleteData('joueurs', array('id_reservation' => $data['id']));
		$datas = array(
			'rPop' => $data['txt'],
	...
	public function deleteIndispo()
	{
		$post = $this->input->post('message');
		$data = json_decode($post,TRUE);
		$this->UserreservationModel->deleteData('indispos', array('id' => $data['id']));
		$datas = array(
			'rPop' => $data['txt'],
			'rPopTitle' => $data['titre'],
			'rPopClass' => (isset($data['class']))?$data['class']:''
	...
	public function infos()
	{
		...
		{
			$this->data['client'] = $this->UserreservationModel->getClientByOrder($this->data['id_reservation']);
			$this->data['joueurs'] = $this->UserreservationModel->getList('joueurs', array('id_reservation' => $this->data['id_reservation']));
			$this->data['reservation'] = $this->UserreservationModel->getData('reservation', array('id' => $this->data['id_reservation']));
		}
	...
	public function infos_up()
	{
		...
		{
			$this->data['client'] = $this->UserreservationModel->getClientByOrder($this->data['id_reservation']);
			$this->data['joueurs'] = $this->UserreservationModel->getList('joueurs', ['id_reservation' => $this->data['id_reservation']]);
			$this->data['reservation'] = $this->UserreservationModel->getData('reservation', ['id' => $this->data['id_reservation']]);
			$this->data['infos_societe'] = $this->UserreservationModel->getData('infos_societe', ['id' => $this->data['reservation']->societe]);
	...
	public function add()
	{
		...
		$test = $this->UserreservationModel->getData('client', array('email' => $data['email']));
		$dernieremodif = time();
		...
		{
			$id = $test->id;
			$this->UserreservationModel->updateData('client', $datas, array('id' => $id));
		}
		else
		{
			$id = $this->UserreservationModel->setData('client', $datas, true);
		}
		...
		if ($data['societe'] == 1) {
			...
			$test_societe = $this->UserreservationModel->getData('infos_societe', array('id' => $data['id_societe']));
			if ($test_societe !== FALSE) {
				$id_societe = $test_societe->id;
				$this->UserreservationModel->updateData('infos_societe', $infos_societe, array('id' => $id_societe));
			}
			else {
				$id_societe = $this->UserreservationModel->setData('infos_societe', $infos_societe, true);
			}
		}
		...
		$salle = $this->UserreservationModel->getData('salle', array('id' => $data['salle']));
		$dernieremodif = time();
		$formatmodif = date('Y-m-d H:i:s', $dernieremodif);
		$resa = array(
			'id_client' => $id,
		...
		if($new_id = $this->UserreservationModel->setData('reservation', $resa, true))
		{
			...
			for($i = 1; $i < PLAYERS_MAX; $i++)
			{
				if($data['joueur' . $i] != '')
				{
					...
					$this->UserreservationModel->setData('joueurs', $joueur);
					if ($data['envoimail'] == 'Oui') {
					// envoi du mail au joueur
						$message = $this->UserreservationModel->getData('email', array('action' => 'player_play_confirm'));
						$sujet = $message->sujet;
			...
			if ($data['envoimail'] == 'Oui') {
				...
				$message = $this->UserreservationModel->getData('email', array('action' => $action));
				$sujet = $message->sujet;
				$content = $message->message;
				$mess_datas = array(
					'dest' => $data['email'],
				...
				modules::run('message', json_encode($mess_datas));
				$message = $this->UserreservationModel->getData('email', array('action' => 'client_play_confirm'));
				$sujet = $message->sujet;
				$content = $message->message;
				$mess_datas = array(
					'dest' => $data['email'],
	...
	public function update()
	{
		...
		$client = $this->UserreservationModel->getData('reservation',['id' => $data['id']],'id_client');
		if($data['password'] != '' && md5($data['password']) != $data['old_pass'])
		{
			$datas['password'] = md5($data['password']);
		}
		$this->UserreservationModel->updateData('client', $datas, array('id' => $client->id_client));
		...
		if($this->UserreservationModel->updateData('reservation', $resa, array('id' => $data['id'])))
		{
			$this->UserreservationModel->deleteData('joueurs', array('id_reservation' => $data['id']));
			// renseignement autres joueurs
			for($i = 1; $i < PLAYERS_MAX; $i++)
			{
				if($data['joueur' . $i] != '')
				{
					...
					if ($data['envoimail'] == 'Oui') {
						// envoi du mail au joueur
						$message = $this->UserreservationModel->getData('email', array('action' => 'player_modif_confirm'));
						$sujet = $message->sujet;
						$content = $message->message;
						$mess_datas = array(
							'dest' => $data['joueur' . $i],
	...
	public function calendar_old()
	{
		...
		$this->data['resas'] = $this->UserreservationModel->getListByDate($salle, $start_search, $end_search);
		if($this->is_ajax())
		{
...
/* 			if ($this->$data['modifresa'] == 'ok') {
				$tempresa = $this->UserreservationModel->getData('reservation', array('id' => $data['id_reservation']));
		...
		$start_search = date('Y-m-d', $this->data['start']);
		$end_search = date('Y-m-d', $this->data['next']);
		$horaire = $this->UserreservationModel->getHoraireSet($salle,$start_search,$end_search);
		$tab = [];
		$this->data['resas'] = $this->UserreservationModel->getListByDate($salle, $start_search, $end_search);
		$this->data['indisps'] = $this->UserreservationModel->getIndispoListByDate($salle, $start_search, $end_search);
	...
	private function change_horaire(){
		$data = $this->UserreservationModel->getList('horaire');
		foreach ($data as $value) {
			$this->UserreservationModel->updateData('horaire',['hor_start' => str_replace(':', 'h', $value->hor_start)], ['hor_id' => $value->hor_id]);
		}
		$data = $this->UserreservationModel->getList('reservation');
		foreach ($data as $value) {
			$this->UserreservationModel->updateData('reservation',['horaire' =>  (strlen($value->horaire) < 3? $value->horaire.'h00': $value->horaire) ], ['id' => $value->id]);
		}
	}
	...
	{
		$verif = $this->UserreservationModel->getData('reservation', array('jour' => $jour, 'horaire' => $horaire, 'id_salle' => $salle));
		$verif2 = $this->UserreservationModel->getData('reservation', array('jour' => $jour, 'horaire' => $horaire, 'id_salle' => $salle, 'id_client' => 0));
		if($verif === FALSE)
		{
			return TRUE;
		}
	...
	public function paid()
	{
		$post = $this->input->post('message');
		$data = json_decode($post,TRUE);
		$this->UserreservationModel->updateData('reservation', array('regle' => 1), array('id' => $data['id']));
		$datas = array(
	...
	public function getNbmax($id = 1, $nb = 0){
		...
		$res = $this->UserreservationModel->getData('salle', ['id' => $id], ['nbmin','nbmax']);
	...
	public function setIndispo(){
		if( $this->input->post('message') )
		{
			...
			if($data['salle'] == 'all'){
				// $id = $this->UserreservationModel->getData('salle',['active' => '1'],'*');
				$id = $this->UserreservationModel->getList('salle',['active' => '1']);
			} else {
				$id = (int)$data['salle'];
			}
			$dernieremodif = time();
...
/* 				'id_client' => 0,
				...
				if(strlen($indispo) == 10){
					...
					if (is_int($id)){	
							$resa['id_salle'] = $id;
							$salledata = $this->UserreservationModel->getData('salle', array('id' => $id));
							$resa['scenario'] = $salledata->nom;
							$hset = $this->UserreservationModel->getHoraireSet($resa['id_salle'],$ishoraireset,$ishoraireset);
							
							foreach ($hset['set'] as $horairesalle) {
								$resa['horaire'] = $horairesalle -> hor_start;
								$this->UserreservationModel->setData('indispos',$resa,true);								
							}
						...
						foreach($id as $salle){
							...
							$salledata = $this->UserreservationModel->getData('salle', array('id' => $salle->id));
							$resa['scenario'] = $salledata->nom;
							$hset = $this->UserreservationModel->getHoraireSet($resa['id_salle'],$ishoraireset,$ishoraireset);
		...
		public function loadComment() {
			...
			$resa = $this->UserreservationModel->getData('reservation', array('id' => $data['id']));
		...
		public function loadCompanyinfos() {
			...
			$infos = $this->UserreservationModel->getData('infos_societe', array('id' => $data['id']));
		...
		foreach ($ficheslist as $resaid) {
			...
			$infosresa = $this->UserreservationModel->getData('reservation',array('id' => $resaid));
			$dateformat = substr($infosresa->jour,8,2).'/'.substr($infosresa->jour,5,2).'/'.substr($infosresa->jour,0,4); 
			$infosclient = $this->UserreservationModel->getData('client',array('id' => $infosresa->id_client));
			$infosjoueurs = $this->UserreservationModel->getList('joueurs',array('id_reservation' => $resaid));
			$emailjoueurs = ['','','','',''];
			...
			$pdf->Cell(9);
			$pdf->Image('http://'.$_SERVER['SERVER_NAME'].'/assets/img/logosra2.jpg',225,14,58,29,'JPG');
			$pdf->Ln(32);
