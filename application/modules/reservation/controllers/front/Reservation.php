...
class Reservation extends MY_Controller
{
	...
	public function add()
	{
		...
		// Nettoyage des valeurs saisies
		$data['email'] = self::cleanPostValue($data['email']);
		for($i = 1; $i < PLAYERS_MAX; $i++)
		{
			if($data['joueur' . $i] != '')
			{
				$data['joueur' . $i] = self::cleanPostValue($data['joueur' . $i]);
			}
		}
	...
	private function cleanPostValue($value){
