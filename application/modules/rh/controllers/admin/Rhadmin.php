...
class Rhadmin extends MY_Controller
{
...
/* 		$this->data['offperiod'] = $this->get_off_period();
	...
	{
		$data['stafflist'] = $this->Rhmodel->getList('staff');
		$data['salles'] = $this->Rhmodel->getList('salle');
	...
	public function get_planning_list()
	{
		...
		$data['staffselinfos'] = $this->Rhmodel->getData('staff',['id' => $staffselid ]);
		$data['stafflist'] = $this->Rhmodel->getList('staff');
		$data['staffplanning'] = $this->Rhmodel->getList('staff_planning');
		$data['staffplanningset'] = $this->Rhmodel->getList('staff_planning_set');
		

				$data['weekshours'] = $this->Rhmodel->getWeeksHours($staffselid);
	...
	public function load_planning_form()
	{
		...
		$data = json_decode($post,TRUE);
		$setid = $data['id'];
		$data['staffplanset'] = $this->Rhmodel->getData('staff_planning_set',['id' => $setid ]);
		//$data['staffset'] = $this->Rhmodel->getData('staff_planning_set',['id' => $setid ]);
		$data['staffplanning'] = $this->Rhmodel->getList('staff_planning',['id_set' => $data['id']]);
		$staffid = $data['staffset']->id_staff;
		$data['staffselinfos'] = $this->Rhmodel->getData('staff',['id' => $staffid ]);
	...
	public function get_global_planning()
	{
		...
		$this->data['stafflist'] = $this->Rhmodel->getList('staff');
		$this->data['nbstaff'] = count($this->data['stafflist']);
		$startdayformat = date('Y-m-d',$this->data['start']);
		foreach ($this->data['stafflist'] as $staff) {
			
			$excepplanset = $this->Rhmodel->isPlanningExcep($staff->id,$startdayformat);	
			$recurplanset = $this->Rhmodel->isPlanningRecur($staff->id,$startdayformat);
			$permplanset = $this->Rhmodel->isPlanningPerm($staff->id,$startdayformat);
			
			if ($excepplanset) {
				$this->data['planningok'][$staff->id] = $this->Rhmodel->getList('staff_planning', array('id_set' => $excepplanset[0]->id));
			}
			else if ($recurplanset) {
				$this->data['planningok'][$staff->id] = $this->Rhmodel->getList('staff_planning', array('id_set' => $recurplanset[0]->id));
			}
			else if ($permplanset) {
				$this->data['planningok'][$staff->id] = $this->Rhmodel->getList('staff_planning', array('id_set' => $permplanset[0]->id));
			}
	...
	public function get_off_period()
	{
		...
		$staffselid = ($data['staffselected'])?$data['staffselected']:1;
		$data['staffselinfos'] = $this->Rhmodel->getData('staff',['id' => $staffselid ]);
		$data['staffoff'] = $this->Rhmodel->getList('staff_off_period',['id_staff' => $staffselid ]);
		$data['stafflist'] = $this->Rhmodel->getList('staff');
		if($this->is_ajax())
		{
	...
	public function get_extra_period()
	{
		...
		$staffselid = ($data['staffselected'])?$data['staffselected']:1;
		$data['staffselinfos'] = $this->Rhmodel->getData('staff',['id' => $staffselid ]);
		$data['staffextra'] = $this->Rhmodel->getList('staff_extra_period',['id_staff' => $staffselid ]);
		$data['stafflist'] = $this->Rhmodel->getList('staff');
		if($this->is_ajax())
		{
	...
	public function load_staff_form()
	{
		$post = $this->input->post('message');
		$data = json_decode($post,TRUE);
		if ($data['id'] != '') $data['staffmodifinfos'] = $this->Rhmodel->getData('staff',['id' => $data['id']]);
		$data['salles'] = $this->Rhmodel->getList('salle');
	...
	public function update()
	{
		...
		if($data['id'] != ''){			 
			$result = $this->Rhmodel->updateData('staff', $datas, array('id' => $data['id']));
		}
		else{
			$result = $this->Rhmodel->setData('staff', $datas);
		}
		unset($datas);
		if($result)
		{
	...
	public function updatePlanning()
	{
		...
		if($data['id'] != ''){			 
			$result = $this->Rhmodel->updateData('staff_planning_set', $datas, array('id' => $data['id']));
			$result2 = $this->Rhmodel->deleteData('staff_planning', array('id_set' => $data['id']));
		}
		else{
			$result = $this->Rhmodel->setData('staff_planning_set', $datas,TRUE);
			$result2 = true;
		}
		unset($datas);
		...
		foreach ($plageinfos as $pi) {
			if ($pi != '') {
				...
				$result3 = $this->Rhmodel->setData('staff_planning', $datas);
			}
		}
		unset($datas);
	...
	public function delete()
	{
		$post = $this->input->post('message');
		$data = json_decode($post,TRUE);
		$this->Rhmodel->deleteData('staff', array('id' => $data['id']));
		$datas = array(
			'rPop' => $data['txt'],
			'rPopTitle' => $data['titre'],
			'rPopClass' => (isset($data['class']))?$data['class']:''
	...
	public function delete_planning()
	{
		$post = $this->input->post('message');
		$data = json_decode($post,TRUE);
		$this->Rhmodel->deleteData('staff_planning_set', array('id' => $data['id']));
		$this->Rhmodel->deleteData('staff_planning', array('id_set' => $data['id']));
		$datas = array(
			'rPop' => $data['txt'],
			'rPopTitle' => $data['titre'],
			'rPopClass' => (isset($data['class']))?$data['class']:''
	...
	public function testPlanAlreadyOn() {
		...
		$resultsperm = $this->Rhmodel->getData('staff_planning_set',$donneestest);
	...
	while ($finished == false) {
			...
			$salles = $this->Rhmodel->getList('salle',$searchsalle);
			
			if (!$gmdejapris) $gmdejapris = array();
			$searchstaff = 'gm != ""';
			foreach($gmdejapris as $gmpris) $searchstaff .= ' AND id != '.$gmpris;
			$staff = $this->Rhmodel->getList('staff',$searchstaff,'*','FIELD (gm_prio, "'.$salle.'") DESC');
		...
		//Création d'un tableau contenant les GM disponibles. Clé = id du gm. Valeur = salles masterisées ------------------------------------			
			...
			$value = "";
			foreach ($staff as $s) {
				$excepplanset = $this->Rhmodel->isPlanningExcep($s->id,$startdayformat);	
				$recurplanset = $this->Rhmodel->isPlanningRecur($s->id,$startdayformat);
				$permplanset = $this->Rhmodel->isPlanningPerm($s->id,$startdayformat);
				...
				if ($excepplanset) {
					foreach ($excepplanset as $e) {
						...
						$excepplan = $this->Rhmodel->getList('staff_planning',array('jour' => $weekday,'id_set'=> $e->id));
						//si oui
						if ($excepplan) {
				...
				//est-ce que ce collaborateur a un planning de type "une semaine sur deux" défini pour la semaine concernée ?
					...
					foreach ($recurplanset as $r) {
						...
						$recurplan = $this->Rhmodel->getList('staff_planning',array('jour' => $weekday,'id_set'=> $r->id));
						//si oui
						if ($recurplan) {
				...
				//est-ce que ce collaborateur a un planning de type permanent défini pour la semaine concernée ?
					...
					foreach ($permplanset as $p) {
						...
						$permplan = $this->Rhmodel->getList('staff_planning',array('jour' => $weekday,'id_set'=> $p->id));
						//si oui
						if ($permplan) {
		...
		//Fin de création d'un tableau contenant les gm disponibles classés par nombre de salles qu'ils peuvent masteriser. Clé = integer. Valeur = id du gm--------------------
			...
			foreach ($whomasters as $key => $value) {
				...
				$resas = $this->Rhmodel->getData('reservation','id_salle = "'.$key.'" AND id_client != 0 AND jour = "'.$jour.'" AND horaire = "'.$heure.'"');
				$staffaffiche = $this->Rhmodel->getData('staff',array('id' => $whomasters[$key][0]));
				if($resas != "") {
					$sallesreservees[] = $key;
		...
		$rooms = $this->Rhmodel->getList('salle',array('active' => 1));
	...
	public function isFreeGM($jour,$heure) {
