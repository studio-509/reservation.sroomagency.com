...
/* 		var required = _tools.required('form_admin_games');
		...
		*/
		...
		//if(required == 0 && format_email == 0){
			...
			_Ajax.send('/usergames/admin/Usergamesadmin/update', datas);
		//} 	
	});
	...
	$('body').on('click','.closePopin',function(){	
		...
		_Ajax.reload('/usergames/admin/Usergamesadmin/search_games_list','tab_games_search',datas);
...
var fn_games = {	
	del : function(id){
			...
			_inPop.open('/usergames/admin/Usergamesadmin/delete', datas, 'alerte');
			$('#_tr'+id).slideUp(500);
		}
};
