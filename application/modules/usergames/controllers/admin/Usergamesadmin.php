...
class Usergamesadmin extends MY_Controller
{
	function __construct(){
		...
		die();
		$this->load->model('UsergamesModel');
	...
	public function search_games_list(){
		...
		$this->data['games'] = $this->UsergamesModel->getGamesList($data['search'],$data['searchstart'],$data['searchend']);
		$this->data['salles'] = $this->UsergamesModel->getList('salle');
	...
	public function delete(){
		$post = $this->input->post('message');
		$data = json_decode($post,TRUE);
		$this->UsergamesModel->deleteData('infos_partie', array('id' => $data['id']));
		$this->UsergamesModel->deleteData('agent', array('id_partie' => $data['id']));
		$datas = array(
			'rPop' => $data['txt'],
	...
	public function infos(){
		...
		$post = $this->input->post('message');
		$data = json_decode($post,TRUE);
		$data['game'] = $this->UsergamesModel->getGame($data['idresa']);
		$data['resajoueurs'] = $this->UsergamesModel->getList('joueurs',array('id_reservation' => $data['idresa']));
		$data['infosjoueurs'] = $this->UsergamesModel->getList('agent',array('id_partie' => $data['game']->id));
	...
	public function update(){
		...
		if($data['id'] != ''){			
			$result1 = $this->UsergamesModel->updateData('infos_partie', $datas, array('id' => $data['id']));
			$idpartie = $data['id'];
		}
		else {
			$result1 = $this->UsergamesModel->setData('infos_partie', $datas, TRUE);
			$idpartie = $result1;
		}
		unset($datas);  
		...
		foreach ($infos_joueurs as $infos) {
			...
			if($inf[0] != 'no'){			
				$result = $this->UsergamesModel->updateData('agent', $datas, array('id' => $inf[0]));
			}
			else {
				$result = $this->UsergamesModel->setData('agent', $datas);
			}
	...
	public function getRank($resaid) {
		$infos_partie = $this->UsergamesModel->getGame($resaid);
		if ($infos_partie->tps_jeu == "00:00:00") return "n/a";
		else {
			$rank = $this->UsergamesModel->getRanking($infos_partie->id_salle,$infos_partie->tps_jeu);
			return $rank; 
		}
