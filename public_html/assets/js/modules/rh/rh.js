$(document).on('ready',function() {
	...
	*/
	$('body').on('click','a.closePopin', function(){
		if (($('#_popin_title').html() == "Modification des données d'un collaborateur")||($('#_popin_title').html() == "Ajout d'un collaborateur")) {
			var datas = {};
			_Ajax.reload('/rh/admin/Rhadmin/get_list_personnel','bloc_staff_list',datas);
		...
		if (($('#_popin_title').html() == "Modification d'un planning")||($('#_popin_title').html() == "Ajout d'un planning")||($('#_popin_title').html() == "Suppression planning")) {
			...
			_Ajax.reload('/rh/admin/Rhadmin/get_planning_list','bloc_staff_planning_list',datas);			
		}
    });
	/**
	* Gestion onglets
	...
	*/
	$(document).on('click', '._staff_add_update', function() {
		...
		_inPop.open('/rh/admin/Rhadmin/load_staff_form', datas, 'alerte', 'fn_staff.update()');
	});
	...
	*/
	$(document).on('click', '#_submit_staff_addmodif_form', function(){
		...
		if(required == 0){
			_Ajax.send('/rh/admin/Rhadmin/update',datas);
		} 	
	});
	...
	*/
	$(document).on('click', '._staff_planning_add_update', function() {
		...
		_inPop.open('/rh/admin/Rhadmin/load_planning_form', datas, 'alerte', 'fn_staff.update()');
	});
	...
	*/
	$(document).on('change','#dateplanning',function(){
		var date = $(this).val();
		var datas = {'date':date};
		_Ajax.reload('/rh/admin/Rhadmin/get_week_number','numberofweek',datas);
	});
	...
	*/
	$(document).on('click','#_submit_planningset_addmodif_form',function(){
		...
		if (id == '') {
			...
			jQuery.ajax({
				type: 'POST',
				url: '/rh/admin/Rhadmin/testPlanAlreadyOn',
				data: {
					'id':staffid,
					'type':type,
					'date':date,
		...
		if(startok == true){
				...
				_Ajax.send('/rh/admin/Rhadmin/updatePlanning',datas);
		}
		else {
	...
	*/
	$(document).on('change','#staff_off_select',function(){
		...
		var datas = {'staffselected':staffselected};
		_Ajax.reload('/rh/admin/Rhadmin/get_off_period','bloc_staff_off_period',datas);
	});
	...
	*/
	$(document).on('change','#staff_extra_select',function(){
		...
		var datas = {'staffselected':staffselected};
		_Ajax.reload('/rh/admin/Rhadmin/get_extra_period','bloc_staff_extra_period',datas);
	});
	...
	*/
	$('body').on('click', '._cal_drop', function(){
		...
		_Ajax.reload('/rh/admin/Rhadmin/get_global_planning', 'bloc_staff_global_planning', datas);
	});
...
var fn_rh = {
	del : function(id){
		...
		_inPop.open('/rh/admin/Rhadmin/delete', datas, 'alerte');
		$('#_tr'+id).slideUp(500);
	},
	delplanning : function(id){
		var datas = {"titre":"Suppression planning","txt":"Le planning à bien été supprimé","id":id};
		_inPop.open('/rh/admin/Rhadmin/delete_planning', datas, 'alerte');
		$('#_tr'+id).slideUp(500);
	}
};
